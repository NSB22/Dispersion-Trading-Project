"""
Build the ML feature table from the existing parquets (no new WRDS pull):
spectral features from the cleaned correlation matrix, SPX vol structure,
realised vol / VRP, the component-IV cross-section, and correlation levels. The
label y_spike is future minus current realised correlation, used in training only.
...
D21 = 21              # short dynamics window

# core set for the Gaussian models — a full-cov fit in 32 dims wouldn't survive
# ~2,000 walk-forward observations
...
def build_ml_dataset(
    ...
    # SPX term slope (30d -> 91d ATM pillars, C/P averaged)
    ...
    # SPX realised vol block (vendor closes, same source as the strikes)
    ...
    # component-IV cross-section (weighted, renormalised)
    ...
    # assemble everything on the date spine
    m = (
    ...
    out["f_vix"] = m["vix"] / 100.0                 # VIX in decimal units
    ...
    out["f_drho_imp_21"] = m["rho_implied"].diff(D21)   # the market starting to price stress
    ...
    out["f_sig_rmt_pct"] = (m["f_sig_rmt"].expanding(min_periods=252)
                            .rank(pct=True))          # expanding percentile, no look-ahead
    year = m["date"].dt.year
    out["f_cost_era"] = [
    ...
    # label uses the future, so it's training-only and purged in every fit
    out["y_spike"] = m["rho_trail63"].shift(-HORIZON) - m["rho_trail63"]

    # force float64 everywhere — nullable dtypes break numpy/sklearn
