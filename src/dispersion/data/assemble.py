...
def build_dataset(
    ...
    for i, reb in enumerate(rebals):
        ...
        active = calendar[(calendar >= reb) & (calendar < nxt)]
        if len(active) == 0:
            continue
    ...
    # bounded ffill per (quarter, permno) on the active calendar
    cleaned = []
    for (reb, _), g in iv_components.groupby(["rebalance_date", "permno"]):
        active = calendar[(calendar >= reb) & (calendar < g["date"].max() + pd.Timedelta(days=1))]
