...
- [✅] Multi-agent adversarial QA (6 dimensions): 4 PASS / 2 WARN. Only 1 genuine issue = OptionMetrics gap 27/07→18/08/2020 (vendor outage, zero IV across the whole surface) → accepted + documented (methodology §9b). **WEEK 1 DONE.**
...
- [✅] **Full end-of-W2 audit** (4 independent axes: data, signal, mathematics, consistency): 0 critical findings; ρ_implied recomputed independently (gap ~1e-16); documentation corrections applied; latent fragilities → methodology §9b. **WEEK 2 DONE** (derivation write-up → W5).
...
- [✅] **Fork decided (notebook 04, 8 Jul): premiums = `impl_premium`** (vsurfd re-extraction — naive BS biased by ±5%, SPX implied q 1.04% identical call/put, 100% coverage from 1996); **greeks = analytic BS/Black-76** from IV + forward (`fwdprd`/put-call parity) + `zerocd` (methodology §8b)
...
- [✅] **Bundled dataset rebuild (8 Jul, 2 passes, ~10 min)**: `surface.parquet` (4.9M rows, pillars 10/30/60/91d, 0 NaN, 0 duplicates) + `spots.parquet` (724k secprd closes + cfadj, 169/329 names with splits, SPX verified) + `rates.parquet` (zerocd) + `returns.parquet` (329 permnos from 1995) + §9b hardening + truncated-tail fix. Non-regression: 5 parquets bit-identical; signal moved on 139 days (≤ 8e-3, documented policy finally applied); headline unchanged (+0.079, t=7.4). QA findings: 10d pillar ~37% populated (sub-30d fallback rule to set at engine design); zerocd missing 10 d/7,281 (ffill at consumption).
...
- [✅] **Costs fork decided (8 Jul, notebook 05, methodology §8b)**: **parametric grid calibrated on real opprcd quotes** (3 groups SPX / rnk 1-50 / rnk 51-100 × linear interpolation 1996→2024: 1→0.6% / 7→1.2% / 10→3%); **½-spread × premium per leg at entry** (spread paid once, DMV held-to-maturity convention) + **1 bp** on the notional of each hedge trade; settlement spread-free. Raw opprcd rejected: SPX 2012 EOD artefact (quoted ≠ effective, ~30-50% of the quote) + COVID 2020 cell. Conservative by construction. Sensitivity ±50% + "stress ×2" → W5
...
### RMT (priority) — spec frozen 8 Jul (methodology §8b), module `src/dispersion/rmt/`
...
- [✅] **Two-window justification** written (methodology §8b + §6.3: 63d = window-matching of the priced horizon; 252d = spectral health, q≈0.4) — ready for the write-up.
...
- [✅] **Step 1 (21 Jul): final strategy `v1_rmt+regime` adopted** (regime-only gate, lever 3) — methodology §14.1b.
