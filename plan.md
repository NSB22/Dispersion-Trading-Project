...
- [✅] **Jambe parcimonieuse (21 juil., notebook 10, `n_leg` dans l'engine)** : trader les top-K noms (par capi) au lieu des 100 → coût/trim **3,68 %→3,36 %** (~9 %, spreads small-cap évités). **Net : top-20 bat le full** (Sharpe 0,60 vs 0,57, skew +1,14 vs +1,00) ; **gross : full légèrement mieux** (0,78 vs 0,75, petite erreur de réplication) → **la parcimonie paie NET pas GROSS = réponse RMT aux frictions validée**. Caveat honnête : effet **NON monotone** (n=30/50 sous le full) → l'optimum à 20 est en partie bruit ; message robuste = « ~15-20 noms ≥ 100 net, à coût réduit ». `fig_parsimonious`, `table_parsimonious`. Test unitaire vega-neutralité (38/38).
- [✅] Justification des **deux fenêtres** écrite (README §8bis + §6.3 : 63j = window-matching de l'horizon pricé ; 252j = santé du spectre, q≈0,4) — prête pour la rédaction.
- [✅] Réfs consolidées (README §13) : BBP 2017, Laloux 1999, Potters–Bouchaud 2020, Kritzman (turbulence/absorption), López de Prado.

### ML (enrichi le 17 juil. — on a le temps : régimes via HMM/GMM en plus du supervisé)
- [✅] Features **spectrales** (rôle B) construites (`rmt_daily.parquet` + `ml/features.py`) : λ₁/N, λ₂/N, absorption, K, Δλ₁, rotation, entropie, participation ratio, turbulence de Mahalanobis + VIX/VRP/vol.
- [✅] Cible **spike de corrélation** explorée (levier 1 : spike quotidien prévisible, corr OOS +0,40 ; méta-label sur le trade adopté). Objectif skew/drawdown confirmé (la porte régime-seul coupe la queue).
...
- [✅] **Menu d'améliorations ML présenté + tranché (17 juil.)** : architecture = **méta-labeling veto** (v1_rmt propose, le ML veto — *choix NON définitif, à réévaluer* ; porte directe en comparaison si temps) ; retenus dès S4 : **labels continus** (spike Δρ̄ forward), **porte coût-aware** (edge prédit > coût d'époque), **ensembling GMM/HMM/XGB**, **calibration isotonique** ; reportés S5 : CPCV.
...
- [✅] **Étape 1 (21 juil.) : stratégie finale `v1_rmt+regime` adoptée** (porte régime-seul, levier 3) — README §14.1bis.
- [✅] **Étape 2 — robustesse (21 juil., notebook 10)** : **prime par sous-période (Newey–West)** significative 1996–2019 (t 3,3–4,9) → **comprimée post-2020 (t=1,19)** ✓ prédiction limits-to-arbitrage. **Nuance honnête** : l'avantage Sharpe du gating RMT n'est PAS uniforme (fort 2004-07 ; **v0 gagne 2008-12 et 2013-19**) → le vrai gain robuste = forme de queue (skew/DD), pas le Sharpe par époque. **Sensibilité robuste** : Sharpe net 0,46–0,67 (coûts ±50 %), 0,57–0,58 (veto q), 0,54–0,56 (porte q). Figures/tables : `fig_subperiods`, `table_{premium_subperiods,strategy_subperiods_sharpe,sensitivity}.csv`.
- [✅] **Analyse Vanna/Volga (21 juil., notebook 10 + `utils/greeks`)** : formes fermées Volga (∂vega/∂σ) et Vanna (∂vega/∂S) ajoutées + testées (différences finies, annulation au strike d2=0). **Narratif** : à l'entrée le straddle ATM est ~2nd-order-neutre (Volga/Vanna ≈ 0), mais c'est une propriété de 1er ordre — quand le spot dérive et que la position vieillit, Volga/Vanna croissent → la vega-neutralité s'érode. Empirique : vega du book dérive de ~3600 au Q1-2018 (la neutralité se défait dans le spike de corrélation). `fig_vanna_volga`. README §14.4.
- [✅] **Robustesse MFIV (21 juil., notebook 11)** : MFIV reconstruite sur la jambe indice (surface SPX complète, 34 piliers/date, Carr–Madan style VIX, 116 rebal.). **MFIV > ATM de +1,2 pt de vol** (19,7 % vs 18,5 %, skew put) sur **100 % des dates** → ρ_implied **sous-estimée**, biais **Δρ ≈ +0,066 (toujours positif)**, ρ passe de 0,43 à ~0,50. **Le proxy ATM est donc CONSERVATEUR** : notre prime est une borne basse, la vraie (MFIV) serait plus grande → renforce la thèse. Nuance : effet jambe-indice-seule = borne HAUTE du biais (les composantes ont aussi un skew, plus plat, qui compenserait partiellement — mais net positif car skew indice >> single-name, mécanisme DMV). `fig_mfiv_bias`, `table_mfiv_bias`.
- [ ] Rédaction : Intro/Client Spec → Théorie → Données & Méthodo → Résultats → Limites → Conclusion
- [✅] **Narratif** — arc complet articulé (README §14.1) : la prime existe (DMV + nos 29 ans) → les frictions la tuent (Table V) → RMT (estimateur 252j + jambe parcimonieuse) + ML (nul honnête, régime-seul coupe la queue) = réponses aux frictions. Prêt pour la rédaction.
- [✅] Figures soignées (9 figures niveau papier, en anglais) + reading list intégrée (README §13).
- [✅] **Revue complète A→Z (21 juil.)** : `main.py` écrit (pipeline WRDS→backtests), lancé — reproduit tous les headlines exacts ; non-régression 13/14 parquets bit-à-bit (1 à 1e-9, bruit régime) ; 39 tests revus et verts ; warning bénin corrigé. **Commentaires/docstrings humanisés** (16 modules + main + tests, ton naturel, 0 résidu FR, tests toujours verts).
- [ ] Nettoyer le repo (final : `git add`, .gitignore) + rédaction
